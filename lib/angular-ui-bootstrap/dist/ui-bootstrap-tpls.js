...
.controller('UibPaginationController', ['$scope', '$attrs', '$parse', 'uibPaging', 'uibPaginationConfig', function($scope, $attrs, $parse, uibPaging, uibPaginationConfig) {
  ...
  // Create pages object used in template
  ...
  function getPages(currentPage, totalPages) {
    ...
    if (isMaxSized) {
      if (rotate) {
        // Current pages is displayed in the middle of the visible ones
        ...
        // Adjust last pages if limit is exceeded
        endPage = Math.min(startPage + maxSize - 1, totalPages);
      }
    }

    // Add pages number links
    for (var number = startPage; number <= endPage; number++) {
      var page = makePage(number, pageLabel(number), number === currentPage);
      pages.push(page);
    }

    // Add links to move between pages sets
      ...
      $q.when(parserResult.source(originalScope, locals)).then(function(matches) {
        ...
        if (onCurrentRequest && hasFocus) {
          if (matches && matches.length > 0) {
            ...
            //with matches as a pop-up might be absolute-positioned and position of an input might have changed on a pages
...
angular.module("uib/template/pager/pager.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/pager/pager.html",
    "<li ng-class=\"{disabled: noPrevious()||ngDisabled, previous: align}\"><a href ng-click=\"selectPage(pages - 1, $event)\" ng-disabled=\"noPrevious()||ngDisabled\" uib-tabindex-toggle>{{::getText('previous')}}</a></li>\n" +
    "<li ng-class=\"{disabled: noNext()||ngDisabled, next: align}\"><a href ng-click=\"selectPage(pages + 1, $event)\" ng-disabled=\"noNext()||ngDisabled\" uib-tabindex-toggle>{{::getText('next')}}</a></li>\n" +
    "");
}]);
