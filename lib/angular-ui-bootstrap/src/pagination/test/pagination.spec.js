describe('pagination directive', function() {
  ...
  it('has the number of the pages as text in each pages item', function() {
    for (var i = 1; i <= 5; i++) {
      expect(getPaginationEl(i).text()).toEqual(''+i);
    }
  });

  it('sets the current pages to be active', function() {
    expect(getPaginationEl($rootScope.currentPage).hasClass('active')).toBe(true);
  });

  it('disables the "previous" link if current pages is 1', function() {
    updateCurrentPage(1);
    expect(getPaginationEl(0).hasClass('disabled')).toBe(true);
  });

  it('disables the "next" link if current pages is last', function() {
    updateCurrentPage(5);
    expect(getPaginationEl(-1).hasClass('disabled')).toBe(true);
  });

  it('changes currentPage if a pages link is clicked', function() {
    clickPaginationEl(2);
  ...
  it('does not change the current pages on "previous" click if already at first pages', function() {
    updateCurrentPage(1);
    clickPaginationEl(0);
    expect($rootScope.currentPage).toBe(1);
  });

  it('does not change the current pages on "next" click if already at last pages', function() {
    updateCurrentPage(5);
    clickPaginationEl(-1);
  ...
  it('does not change the current pages when `total-items` changes but is valid', function() {
  ...
  it('should blur a pages link after it has been clicked', function() {
    body.append(element);
  ...
  describe('`items-per-pages`', function() {
    beforeEach(function() {
      $rootScope.perpage = 5;
      element = $compile('<ul uib-pagination total-items="total" items-per-pages="perpage" ng-model="currentPage"></ul>')($rootScope);
      $rootScope.$digest();
    });
    ...
    it('selects the last pages when current pages is too big', function() {
    ...
    it('displays a single pages when it is negative', function() {
      $rootScope.perpage = -1;
      $rootScope.$digest();
  ...
  describe('when `pages` is not a number', function() {
  ...
  describe('with `max-size` option', function() {
    ...
    it('shows the pages number even if it can\'t be shown in the middle', function() {
      updateCurrentPage(1);
      expect(getPaginationEl(1)).toHaveClass('active');
    ...
    it('shows the pages number in middle after the next link is clicked', function() {
      updateCurrentPage(6);
      clickPaginationEl(-1);
    ...
    it('shows the pages number in middle after the prev link is clicked', function() {
      updateCurrentPage(7);
      clickPaginationEl(0);
    ...
    it('should not display pages numbers, if max-size is zero', function() {
      $rootScope.maxSize = 0;
      $rootScope.$digest();
    ...
    it('should blur pages link when visible range changes', function () {
      body.append(element);
  ...
  describe('with `force-ellipses` option', function() {
    ...
    it('should display an ellipsis on the right if the last displayed pages\'s number is less than the last pages', function() {
      updateCurrentPage(1);
      expect(getPaginationAsText()).toBe('Previous, 1, 2, 3, 4, 5, ..., Next');
    });

    it('should display an ellipsis on the left if the first displayed pages\'s number is greater than 1', function() {
      updateCurrentPage(10);
  ...
  describe('with `boundary-link-numbers` option', function() {
    ...
    it('should not display an ellipsis on the left if the start pages is 2', function() {
      updateCurrentPage(4);
      expect(getPaginationAsText()).toBe('Previous, 1, 2, 3, 4, 5, 6, ..., 10, Next');
    });

    it('should not display an ellipsis on the left if the start pages is 3', function() {
      updateCurrentPage(5);
      expect(getPaginationAsText()).toBe('Previous, 1, 2, 3, 4, 5, 6, 7, ..., 10, Next');
    });

    it('should not display an ellipsis on the right if the end pages is totalPages - 1', function() {
      updateCurrentPage(7);
      expect(getPaginationAsText()).toBe('Previous, 1, ..., 5, 6, 7, 8, 9, 10, Next');
    });

    it('should not display an ellipsis on the right if the end pages is totalPages - 2', function() {
      updateCurrentPage(6);
      expect(getPaginationAsText()).toBe('Previous, 1, ..., 4, 5, 6, 7, 8, 9, 10, Next');
    });

    it('should not display any ellipses if the number of pages <= maxsize + 4 and current pages is in center', function() {
  ...
  describe('with `max-size` option & no `rotate`', function() {
    ...
    it('shows only the next ellipsis element on first pages set', function() {
      updateCurrentPage(3);
      expect(getPaginationEl(1).text()).toBe('1');
    ...
    it('shows only the previous ellipsis element on last pages set', function() {
  ...
  describe('pagination directive with `boundary-links`', function() {
    ...
    it('disables the "first" & "previous" link if current pages is 1', function() {
      updateCurrentPage(1);
    ...
    it('disables the "last" & "next" link if current pages is num-pages', function() {
      updateCurrentPage(5);
    ...
    it('does not change the current pages on "first" click if already at first pages', function() {
      updateCurrentPage(1);
      clickPaginationEl(0);
      expect($rootScope.currentPage).toBe(1);
    });

    it('does not change the current pages on "last" click if already at last pages', function() {
      updateCurrentPage(5);
      clickPaginationEl(-1);
      expect($rootScope.currentPage).toBe(5);
    });
  ...
  describe('pagination directive with just number links', function() {
    ...
    it('does not disable the "1" link if current pages is 1', function() {
      updateCurrentPage(1);
    ...
    it('does not disable the "last" link if current pages is last pages', function() {
      updateCurrentPage(5);
  ...
  describe('with just boundary & number links', function() {
    ...
    it('disables the "first" & activates "1" link if current pages is 1', function() {
      updateCurrentPage(1);
  ...
  describe('`num-pages`', function() {
    ...
    it('shows minimun one pages if total items are not defined and does not break binding', function() {
      $rootScope.total = undefined;
      $rootScope.$digest();
  ...
  describe('override configuration from attributes', function() {
    beforeEach(function() {
      ...
      element = $compile('<ul uib-pagination boundary-links="true" pages-label="pageLabel($pages)" first-text="<<" previous-text="<" next-text=">" last-text=">>" total-items="total" ng-model="currentPage"></ul>')($rootScope);
      $rootScope.$digest();
    });
    ...
    it('has the label of the pages as text in each pages item', function() {
      for (var i = 1; i <= 5; i++) {
